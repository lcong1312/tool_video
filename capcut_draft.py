...
ACP_PYTHONW = Path(r"C:\Program Files\Auto Capcut Pro\python\pythonw.exe")
...
def _builder_python() -> str:
    if LOCAL_ACP_PYTHONW.is_file():
        return str(LOCAL_ACP_PYTHONW)
    if LOCAL_ACP_PYTHON.is_file():
        return str(LOCAL_ACP_PYTHON)
    if ACP_PYTHONW.is_file():
        return str(ACP_PYTHONW)
