...
def _rebuild_response(body: bytes, status_code: int, headers, media_type) -> Response:
    """Build a fresh Response from an already-drained body.

    Drops Content-Length / Content-Encoding headers so starlette can
    recompute them — otherwise the old Content-Length won't match the
    new body length and uvicorn raises
...
class ResponseMiddleware(BaseHTTPMiddleware):
    ...
    async def _process_json_response(self, response, lang: str):
        """处理JSON响应并统一格式"""
        # Drain the body iterator — from this point `response` has an
        # exhausted iterator and can no longer be returned directly, or
        # starlette/uvicorn will raise "Response content shorter than
        # Content-Length" when they try to re-send it.
        body_chunks = [section async for section in response.body_iterator]
        body_bytes = b"".join(body_chunks)

        if not body_bytes:
            return _rebuild_response(
                b"", response.status_code, response.headers, response.media_type
            )
        ...
        try:
            ...
            # 如果响应已经有统一格式，直接返回（rebuild from drained body）
            ...
            return _rebuild_response(
                body_bytes, response.status_code, response.headers, response.media_type
            )
