...
class JianyingController:
    ...
    def get_window(self) -> None:
        ...
        if not self.app.Exists(0):
            raise AutomationError("剪映/CapCut 窗口未找到")

        # 寻找可能存在的导出窗口 (支持 剪映 "导出" 和 CapCut "Export")
        export_window = self.app.WindowControl(searchDepth=1, Name="导出")
        if not export_window.Exists(0):
            export_window = self.app.WindowControl(searchDepth=1, Name="Export")
    ...
    def __jianying_window_cmp(self, control: uia.WindowControl, depth: int) -> bool:
        # 支持 剪映专业版 和 CapCut
        if control.Name not in ["剪映专业版", "CapCut"]:
            return False
